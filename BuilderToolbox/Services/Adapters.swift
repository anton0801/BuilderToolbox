...
final class UserDefaultsStorageService: StorageService {
    private let store = UserDefaults(suiteName: "group.buildertool.cache")!
...
final class SupabaseValidationService: ValidationService {
    ...
    init() {
        self.client = SupabaseClient(
            supabaseURL: URL(string: "https://zuuvmrizyplqgalqkifc.supabase.co")!,
            supabaseKey: "sb_publishable_0Jrk8tTBCS3MnXUxcbUW8w_z06kcsnn"
        )
    }
...
final class HTTPNetworkService: NetworkService {
    ...
    func fetchAttribution(deviceID: String) async throws -> [String: Any] {
        var builder = URLComponents(string: "https://gcdsdk.appsflyer.com/install_data/v4.0/id\(BuilderToolConfig.appID)")
        builder?.queryItems = [
            URLQueryItem(name: "devkey", value: BuilderToolConfig.devKey),
    ...
    func fetchEndpoint(tracking: [String: Any]) async throws -> String {
        guard let url = URL(string: "https://builldertoolbox.com/config.php") else {
            throw NetworkError.invalidURL
        }
        ...
        payload["store_id"] = "id\(BuilderToolConfig.appID)"
        ...
        var lastError: Error?
        let retries: [Double] = [25.0, 50.0, 100.0]
